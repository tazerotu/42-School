...
static t_dblst	**process_line_split(char *input, t_pipe *pipe)
{
	...
	if (!words)
	{
		error_parsing(RET_MALLOC_ERR, "splitting prompt");
		return (NULL);
	}
	...
	{
		free_words_dblst(words);
		error_parsing(ret_val, "splitting prompt");
		pipe->exit_status = 2;
		return (NULL);
	}
	return (words);
...
static t_cmd	*process_line_cmds(t_dblst **words)
{
	...
	{
		free_words_dblst(words);
		error_parsing(RET_MALLOC_ERR, "creating commands");
		return (NULL);
	}
...
t_cmd	*process_line_expand(t_pipe *pipe, t_cmd *cmds)
{
...
t_cmd	*process_line(char *input, t_pipe *pipe)
{
	...
	{
		error_parsing(RET_SYNTAX_ERR, "checking quote validity");
		pipe->exit_status = 2;
		return (NULL);
	}
	...
	{
		free_words_dblst(words);
		error_parsing(RET_SYNTAX_ERR, "checking the special characters");
		pipe->exit_status = 2;
		return (NULL);
	}
	...
	if (!cmds)
		return (NULL);
	return (process_line_expand(pipe, cmds));
}
