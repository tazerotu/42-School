...
/*   Updated: 2024/11/18 20:39:00 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (j < map->x)
		{
			if (map->map[i][j] == 'C')
			{
				img = mlx_xpm_file_to_image(map->mlx.mlx, COIN, &width,
						&height);
				mlx_put_image_to_window(map->mlx.mlx, map->mlx.win, img, (j * PIX), (i * PIX));
...
void exit_init(t_map *map)
{
	...
	mlx_put_image_to_window(map->mlx.mlx, map->mlx.win, img, (map->exit->x * PIX), (map->exit->y * PIX));
	mlx_destroy_image(map->mlx.mlx, img);
}
